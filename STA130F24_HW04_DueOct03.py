...
# Standard deviation measures the spread of each data points in dataset. It measures how different the data points are compare to the mean (average). It shows the degree of variability of the data itself.
# In contrast, the SEM measures how far the sample mean varies from the population mean. It shows the validity of the sample mean as an estimate of the population mean when the process is repeated for a couple of times.
# To summarize, SD can only tell the variability of the data points themselves, but SEM can measure the variability of the set of sample means.Therefore their calculation are different also: SME = SD/root of sample size.
# 
...
# Firstly we should calculated the sample mean of the data, then calculated the standard error of the mean. We can assume the distribution is a normal distribution, then the approximately 95% of the values fall in about 2 standard errors of the mean.Thus we can calculate the confidence interval by (mean - 2 SME, mean + 2 SME).
...
# Firstly we can calculate all the bootstrapped sample means, then we can set the means in an order from small to large. Next, we
...
import numpy as np
def bootstrap_confidence_interval(data, num_bootstrap_samples=1000, alpha=0.05, stat_function=np.mean):
    """
    ...
    stat_function: function, the statistic for which you want to compute the confidence interval
                   (default is np.mean for the population mean)
                   
    Returns: (lower_bound, upper_bound) for the confidence interval
    """
    
    n = len(data)
    
    bootstrap_samples = np.empty(num_bootstrap_samples)
    
    for i in range(num_bootstrap_samples):
        bootstrap_sample = np.random.choice(data, size=n, replace=True)
        bootstrap_samples[i] = stat_function(bootstrap_sample)
...
mean_confidence_interval = bootstrap_confidence_interval(sample_data)
print(f"95% Bootstrap Confidence Interval for the Mean: {mean_confidence_interval}")

median_confidence_interval = bootstrap_confidence_interval(sample_data, stat_function=np.median)
print(f"95% Bootstrap Confidence Interval for the Median: {median_confidence_interval}")
...
# (1)Me:Do you know what is the process of bootsrapping?
#  Friend: I think I've heared about it on my stats lectures.It's a sampling method right?
#  Me: yea.
#  Friend: Oh I remembered. It means during an experiment, they resample for a couple of times on the same data. And the sampling process is completely random. A single data can be chosen twice or more in each resample process I think.My prof said that he loves this technique because it allows estimation of almost any statistic.

# (2)Me: Do you know what's the purpose of it?
# Friend: Emmm...Maybe it can help you get an idea of what the bigger picture might look like without the need to gather a whole bunch of data. Think of it like when we are playing poker cards, you take one card and record what it is, then place it back, then take another. Repeat the process for certain times to see different possible outcomes. The goal is to understand things in the dataset like the average or variation or others, which helps you understand the data better.

# (3)Me:Ok, so let me ask you a specific question, if you had a guess about what the average of a population was, and you had a sample of size n from that population, how could you use bootstrapping to assess whether or not your guess might be plausible?
# Friend: Good question! Let’s say you think the average height of students at UofT is 170cm, but you only measured 100 random students, and their average height turned out to be 174cm. So you want to find out if your guess of 170cm is reasonable or not. This is a situation in which  bootstrapping can be useful: now you take your sample of 100 people and randomly select 100 heights from it for many times. Each time, calculate the average height.Note that repeat is allowed This will give you a whole set of possible averages from your resampled data. With all these averages, you now have a better idea of what the overall average could be, based on your sample. Then you can create a so called confidence interval that covers most of these means, maybe about 95%. If your guess of 170cm lays inside it, it's reasonable then.
...
# Null Hypothesis:
# In this context, the null hypothesis is that the vaccine has no effect on health. This would mean there's no significant difference between the Initial Health Score and the Final Health Score. We will test this using bootstrapping to evaluate if there is a statistically significant improvement in health after the vaccine is administered.
# 
# Data Visualization:
# Start by creating simple plots to visualize the changes in health scores. Box Plot or Histogram to show the distribution of Initial and Final Health Scores.Scatter Plot comparing Initial Health Score vs Final Health Score for each patient. This will provide insight into any visible differences in the health scores pre- and post-vaccine.
# 
# Quantitative Analysis:
# (1) Define improvement.
# Improvement = Final Health Score − Initial Health Score
# If the vaccine is effective, we expect this improvement to be positive on average.
# 
# (2) Bootstrapping Process. We’ll use bootstrapping to estimate the distribution of the average improvement under the null hypothesis.
# -Resample the data: Randomly sample (with replacement) from the data to create new sets of improvements. Each resample should have the same number of patients. Calculate the mean improvement for each bootstrapped sample.Repeat this many times for 10,000 times to build up a distribution of the bootstrapped means.
# (3) Find onfidence Interval. From the bootstrapped means, compute a 95% confidence interval. If the null hypothesis of no effect is true, the average improvement would be zero. If the confidence interval does not contain zero, tehn the vaccine is effective.
# 
# The code is below:
...
plt.figure(figsize=(8, 5))
plt.bar(df['PatientID'], df['HealthChange'], color='skyblue')
plt.title('Change in Health Score by Patient')
plt.xlabel('Patient ID')
...
mean_change_confidence_interval = bootstrap_confidence_interval(health_changes)
print(f"95% Bootstrap Confidence Interval for the Mean Health Change: {mean_change_confidence_interval}")
...
# Here is the link of chatbot conversation:
# https://chatgpt.com/share/66ff6637-91b0-8004-b0e8-ff4c659cb080

# Conversation summarization:
# Python Simulations: You are refining Monte Hall problem code, focusing on simplifying and streamlining it for better clarity. This is based on code shared by Prof. Schwartz.
# 
# Vaccine Data Analysis Assignment for AliTech: You are tasked with evaluating the effectiveness of a new vaccine. We discussed:
# 
# Using bootstrapping to analyze health score data and test the null hypothesis that the vaccine has no effect.
# Visualizing initial and final health scores, calculating the 95% confidence interval for the mean health change, and testing for statistical significance to determine the vaccine’s impact.
# You are working on a clear, documented report involving both code and visualization to support your analysis.
# Both projects involve hypothesis testing, quantitative analysis, and a focus on clarity and code documentation.
